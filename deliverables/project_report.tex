...
\begin{abstract}
    The Variational Quantum Eigensolver (VQE) is a hybrid quantum-classical algorithm used to compute ground state energies of molecular systems. This project implements VQE to calculate the potential energy surface of the hydrogen molecule (H$_2$) across 100 bond lengths using the PennyLane quantum computing framework. We present a baseline serial implementation with a total runtime of 593.95 seconds (9.90 minutes) on HPC infrastructure. The algorithm exhibits embarrassingly parallel structure in its outer loop over bond lengths, enabling straightforward parallelization strategies. We implement and benchmark multiple optimization approaches and present a rigorous \textbf{three-factor speedup analysis}: (1) Optimizer + JIT compilation achieving 4.13× speedup (593.95s → 143.80s), (2) GPU device acceleration achieving 3.60× speedup (593.95s → 164.91s), and (3) MPI parallelization achieving 28.5× speedup relative to the proper Optax+JIT baseline (143.80s → 5.04s). The combined effect yields 117.85× total speedup (593.95s → 5.04s). Notably, CPU+JIT (143.80s) outperforms GPU (164.91s) for our 4-qubit system due to GPU overhead exceeding benefits at small circuit sizes. The optimized MPI implementation reduces runtime from nearly 10 minutes to 5 seconds, making interactive quantum chemistry parameter exploration practical.
\end{abstract}
...
# Cost and Update snippet for Optax
    ...
    new_params = optax.apply_updates(curr\_params,updates) 
...
\subsubsection{Implementation 4: MPI Parallelization}
...
\begin{enumerate}
    ...
    \item \textbf{Factor 2: GPU Device Acceleration (3.60×)}
    \begin{itemize}
        \item Serial PennyLane Adam: 593.95s → GPU lightning.gpu: 164.91s
        \item Limited benefit at 4 qubits due to GPU overhead
        \item CPU+JIT (143.80s) outperforms GPU (164.91s) for small circuits
    \end{itemize}
...
\begin{enumerate}
    \item \textbf{Three-Factor Decomposition}: The 117.85× total speedup decomposes into: (a) 4.13× from optimizer+JIT, and (b) 28.53× from MPI parallelization. This decomposition is critical for understanding where the speedup actually comes from.

    \item \textbf{CPU+JIT vs GPU}: Contrary to expectations, CPU with JIT compilation (143.80s) outperforms GPU (164.91s) for our 4-qubit system. GPU overhead exceeds benefits at small circuit sizes.
    ...
    \item \textbf{Saturation Beyond 16 Processes}: Speedup levels off around 117× for 16 or more processes, showing we've reached the limit of how much this problem can be parallelized for 100 bond lengths.
...
\textbf{GPU Acceleration Results:} The GPU implementation using \texttt{lightning.gpu} on NVIDIA H100 achieved 3.60× speedup (164.91s vs 593.95s). However, the \textbf{key finding} is that CPU+JIT (143.80s) outperforms GPU (164.91s) for our 4-qubit system:
\begin{itemize}
    \item GPU kernel launch overhead exceeds benefit for small 16-dimensional state vectors
    \item JIT enables early convergence, reducing total iterations
    \item GPU per-iteration time is faster (0.0145s vs 0.0204s), but total runtime is longer
    \item This demonstrates that hardware choice must match problem scale
\end{itemize}
...
\textbf{Three-Factor Decomposition:} The total 117.85× speedup decomposes cleanly:
\begin{itemize}
    \item Optimizer+JIT effect: 4.13× (593.95s → 143.80s)
    \item MPI parallelization: 28.53× (143.80s → 5.04s)  
    \item Combined: $4.13 \times 28.53 = 117.85$×
\end{itemize}

\textbf{Efficiency Analysis:} The efficiency values exceeding 100\% (e.g., 851\% for MPI-2) occur because:
\begin{itemize}
    \item Each MPI process performs independent JIT compilation
    \item The workload is embarrassingly parallel (no communication)
    \item Cache effects from smaller working set per process improve performance
    \item These effects combine to produce super-linear scaling
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Three-Factor Speedup Analysis:} We rigorously decomposed the 117.85× total speedup into independent factors:
          \begin{itemize}
              \item \textbf{Factor 1 - Optimizer+JIT}: 4.13× (593.95s → 143.80s) from Optax optimizer and Catalyst JIT compilation
              \item \textbf{Factor 2 - GPU Device}: 3.60× (593.95s → 164.91s) from lightning.gpu on NVIDIA H100
              \item \textbf{Factor 3 - MPI Parallelization}: 28.53× (143.80s → 5.04s) using proper Optax+JIT baseline
          \end{itemize}

    \item \textbf{Critical Finding - CPU+JIT vs GPU:} For our 4-qubit system, CPU with JIT compilation (143.80s) \textbf{outperforms} GPU (164.91s). GPU overhead exceeds benefits at small circuit sizes; GPU advantage increases with qubit count ($>$10 qubits).

    \item \textbf{MPI Excellence:} MPI parallelization achieved 28.53× speedup relative to the proper Optax+JIT baseline through:
          \begin{itemize}
    ...
    \item \textbf{Methodological Contribution:} The three-factor analysis demonstrates the importance of control experiments. Without the Serial Optax+JIT baseline (143.80s), we would have incorrectly attributed all speedup to parallelization rather than the combination of algorithmic and parallel improvements.
    ...
    \item \textbf{Best Practices Identified:} For VQE quantum chemistry calculations:
          \begin{itemize}
              ...
              \item For small circuits ($<$10 qubits), CPU+JIT outperforms GPU
              \item MPI works excellently for embarrassingly parallel parameter sweeps
              \item Match process count to problem size (100 bond lengths ÷ 16 processes $\approx$ 6 per process)
...
\begin{enumerate}
    \item \textbf{Match hardware to problem size}: CPU+JIT outperforms GPU for small circuits ($<$10 qubits). GPU acceleration becomes beneficial for larger circuits where state vector operations dominate over kernel launch overhead.

    \item \textbf{Algorithmic improvements first}: Optimizer choice and JIT compilation had 4.13× impact---always optimize your serial code before parallelizing. The optimizer+JIT effect is independent of and multiplies with parallelization gains.

    \item \textbf{Establish proper baselines}: Our three-factor analysis required the Serial Optax+JIT control experiment. Without it, we would have incorrectly claimed 117× from MPI alone rather than 28.5× from MPI and 4.13× from optimizer+JIT.
    ...
    \item \textbf{Know your saturation point}: For 100 independent calculations, 16-32 processors is optimal; using more processors adds coordination overhead without faster completion.
\end{enumerate}
...
The 117× speedup we achieved shows that practical quantum chemistry calculations can be done today using classical HPC, while also preparing us for future hybrid quantum-classical computing.
