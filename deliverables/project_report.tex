...
The modest speedup shows that GPU overhead is too large for this problem size. JIT compilation (pre-compiling code to run faster) provides some benefit, but the time spent transferring data to and from the GPU and starting GPU calculations limits the performance gains for small quantum circuits.
...
\begin{itemize}
    \item Time spent copying data to and from GPU memory for small quantum circuits (4 qubits)
    \item GPU startup costs taking longer than the actual calculations
    \item Not enough work within a single circuit to keep all GPU cores busy
    \item Small problem size unable to take advantage of the GPU's ability to do many calculations at once
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Match hardware to problem size}: Small circuits work better on CPUs with parallel processing; GPUs aren't worth using until circuits have 10+ qubits with many operations.
